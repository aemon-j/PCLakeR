...
loadPackage <- function(package) {
  ...
  if (suppressWarnings(require(package, character.only = T))) {
    ...
    if (suppressWarnings(require(package, character.only = T))) {
      ## Library loaded
      cat(paste("\nPackage '", package, "' succesfully installed and loaded.\n", sep = ""))
    } else {
      # Library not loaded
...
getForcingAndInterpolate <- function(forcing_name, 
  ...
  ## debug
  #for(i in 1:length(names(lsFORCINGS_prep[[1]]))){
  # print(i)
  #forcing_name = names(lsFORCINGS_prep[[1]])[i]
  # forcing_name <- "mMixDepth" 
  # pathLoc <- pathForc
  # location <- locFORCING
  # metadata = lsINTERMEDIATE[[1]]
  # timesteps = times_forcing

  # forcing_name <- pointer_df$param_names[1]
  # location <- "excel"
  # pathLoc <- pathXls
  # metadata <- pointer_df
  # timesteps <- seq(0, 365*20)
  ...
  if(is.na(suppressWarnings(as.numeric(forcing_file_info)))){
    ...
    #test_x <- as.numeric(as.character(forcing_from_wb$dTime))
    #test_y <- as.numeric(as.character(forcing_from_wb$dValue))
    #test_list_int <- approx(x = test_x, y = test_y, xout = timesteps, method = "linear", rule = 2:1) ## interpolate missing day values linearly
    #test_x[order(test_x)]
...
adjustSedimentParamSettings_Rnet <- function(dfPARAMS, column_name, sediment_type = c("clay", "peat", "sand", "clay_peat", "clay_sand")){
...
## reg expression to make the total extra upstream transport formula's (e.g. wSiTranWHypUpstTotN1)
makeTranUpstTotFormulas <- function(dfSUBSTANCES, sel_node){
  # dfSUBSTANCES <- tran_subst #dfSUBST
  # sel_node <- 11
  ## remove zooplankton fraction
  ## these formula's are only used in the derivatives for the external component of the mass balance error
  ## calculation. In this formula the transport fraction of zooplankton is already accounted for separately.
  dfUpstNames <- data.frame(tran_subst = paste0(dfSUBSTANCES[-grep("Zoo", dfSUBSTANCES$tran_subst), "tran_subst"], "Upst", "N", sel_node))
...
## make the list with auxiliaries
makeAuxNamesRNet <- function(dfAUXILARIES, dfSUBSTANCES, dfNETWORK, vNODES){
# 
#   dfAUXILARIES <- lDATM_SETTINGS$auxils
#   dfSUBSTANCES <- tran_subst
#   dfNETWORK <- pointer_df
#   vNODES <- node_info$box_nr
  ...
  ## remove the section of the added tran_subst names if their node only exists in the column "box_nr_from". 
  ...
  if(length(grep("UpstN", aux_names))>0){
    if(length(unique(setdiff(dfNETWORK$box_nr_from, dfNETWORK$box_nr_to)))>0){
      aux_to_remove <- grep(paste0(paste0("UpstN", unique(setdiff(dfNETWORK$box_nr_from, dfNETWORK$box_nr_to))), collapse = "|"), aux_names)
  ...
  aux_upstTotNames <- gsub("(^w[[:alnum:]]{1,2})(TranW)(Hyp|Epi)(UpstTot)(N[[:digit:]]+)($|.+)",
                           "\\1\\2\\3\\4\\5", 
                           unlist(lapply(unique(dfNETWORK$box_nr_to), function(x) makeTranUpstTotFormulas(dfSUBSTANCES = dfSUBSTANCES, sel_node = x))))
